              ...
              <v-list>
                <v-list-item>
                  <v-list-item-title
                    ...
                    @click="setP1('agree')"
                    >Agree</v-list-item-title
                  >
                </v-list-item>
                <v-list-item>
                  <v-list-item-title
                    style="cursor: pointer"
                    @click="setP1('disagree')"
                    >Disagree</v-list-item-title
                  >
                </v-list-item>
              </v-list>
            </v-menu>
...
export default {
  ...
  methods: {
    async readFile() {
      const response = await axios.get(`assets/agreedisagree.txt`);
    ...
    goNext() {
      if (this.curr_step < this.qData.length) {
        ...
        this.$router.push("/done.agree.disagree");
      }
    },
    goPrev() {
      if (this.curr_step !== 1) {
    ...
    saveData() {
      ...
      if (this.p1.length > 1 && this.p2.length > 1) {
        this.$store.commit("setAgreeDisagree", obj);
      }
    },
    populateData() {
      let agreeDisagree = this.$store.state.agreeDisagree;
      let isFound = false;
      agreeDisagree.forEach((ele) => {
